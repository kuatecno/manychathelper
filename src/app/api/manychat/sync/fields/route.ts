...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = SyncFieldsSchema.parse(body);
    ...
    // Create sync log
    const logId = await syncService.createSyncLog('manual', 'in_progress');

    try {
      // Sync custom fields
      const result = await syncService.syncCustomFields();
